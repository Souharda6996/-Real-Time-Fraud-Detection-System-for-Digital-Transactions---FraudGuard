...
const CATEGORY_COLORS = ['#EF4444', '#F59E0B', '#DC2626', '#8B5CF6', '#EC4899', '#38BDF8', '#22C55E', '#4B5563'];
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14 }}>
        {[
          { label: 'Total Scored', value: kpis.totalToday, color: '#38BDF8' },
          { label: 'Total Blocked', value: kpis.blockedCount, color: '#EF4444' },
          { label: 'Total Reviewed', value: kpis.reviewCount, color: '#F59E0B' },
        ].map(({ label, value, color }) => (
          <div key={label} style={{
            background: '#0F1524', border: '1px solid #232B42', borderRadius: 12,
            padding: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center'
          }}>
            <div style={{ fontSize: 12, color: '#8B93A8', textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 600 }}>{label}</div>
      ...
      <div style={{ background: '#0F1524', border: '1px solid #232B42', borderRadius: 12, padding: '20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 12, flexShrink: 0 }}>
            {[
              { label: 'Fraud Caught', value: `${fraudCaught}%`, color: '#22C55E' },
              { label: 'False Positives', value: `~${falsePositives}/day`, color: '#EF4444' },
              { label: 'F1 Score', value: prPoint?.f1?.toFixed(1), color: '#38BDF8' },
            ].map(({ label, value, color }) => (
              <div key={label} style={{
                background: '#161D30', border: '1px solid #232B42', borderRadius: 8,
        ...
        <div style={{ marginBottom: 16 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8, fontSize: 12 }}>
            <span style={{ color: '#22C55E' }}>← Catch More Fraud (more FP risk)</span>
            <span style={{ color: '#F4F6FB', fontFamily: 'JetBrains Mono, monospace', fontWeight: 700 }}>
              Threshold: {threshold}
            </span>
            <span style={{ color: '#38BDF8' }}>Fewer False Positives →</span>
          </div>
        ...
        <ResponsiveContainer width="100%" height={220}>
          <LineChart data={PR_CURVE} margin={{ top: 5, right: 10, bottom: 5, left: -20 }}>
            ...
            <Line type="monotone" dataKey="precision" stroke="#38BDF8" strokeWidth={2} dot={false} name="Precision" />
            <Line type="monotone" dataKey="recall" stroke="#22C55E" strokeWidth={2} dot={false} name="Recall" />
            <Line type="monotone" dataKey="f1" stroke="#F59E0B" strokeWidth={1.5} dot={false} strokeDasharray="4 3" name="F1 Score" />
            {/* Threshold line */}
            <Line
              type="monotone"
              dataKey={() => null}
        ...
        <div style={{ display: 'flex', gap: 20, justifyContent: 'center', marginTop: 8, fontSize: 11 }}>
          {[{ color: '#38BDF8', label: 'Precision' }, { color: '#22C55E', label: 'Recall' }, { color: '#F59E0B', label: 'F1 Score' }].map(({ color, label }) => (
            <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 5, color: '#8B93A8' }}>
        ...
        <div style={{ background: '#0F1524', border: '1px solid #232B42', borderRadius: 12, padding: '20px' }}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={volumeData} margin={{ top: 5, right: 10, bottom: 5, left: -20 }}>
              ...
              <Area type="monotone" dataKey="legitimate" stackId="1" stroke="#38BDF8" fill="url(#legitGrad)" strokeWidth={1.5} name="Legitimate" />
              <Area type="monotone" dataKey="fraud" stackId="1" stroke="#EF4444" fill="url(#fraudGrad)" strokeWidth={1.5} name="Fraud" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 5, marginTop: 8, maxHeight: 120, overflowY: 'auto' }}>
            {dynamicFraudCategories.map(({ name, value, color }) => (
              <div key={name} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 11 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#8B93A8' }}>
                  <div style={{ width: 8, height: 8, borderRadius: 2, background: color, flexShrink: 0 }} />
                  <span style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 160 }}>{name}</span>
                </div>
                <span style={{ color: '#F4F6FB', fontWeight: 600, fontFamily: 'JetBrains Mono, monospace' }}>{value}%</span>
              </div>
            ))}
          </div>
        </div>
